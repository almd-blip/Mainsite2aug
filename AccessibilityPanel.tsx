...
export default function AccessibilityPanel({ 
  ...
  const toggleModule = (moduleName: string) => {
    const active = settings.activeModules.includes(moduleName)
      ? settings.activeModules.filter(m => m !== moduleName)
      : [...settings.activeModules, moduleName];
    ...
    <div className="space-y-8 text-left" id="accessibility-control-panel" role="region" aria-label="Accessibility and Comfort Control Panel">
      ...
      <div className="border-b pb-4 border-current/10 flex items-start justify-between gap-4" id="acc-header-block">
        <div className="space-y-1" id="acc-header-titles">
          <div className="flex items-center gap-2">
            <Heart className="w-4 h-4 text-[#912A4A] shrink-0" />
          ...
          <p className="text-xs opacity-80 leading-normal max-w-xl" id="acc-desc-main">
            Designed to support diverse cognitive, visual, and motor needs. Customise your interface to create a peaceful, readable, and human-centred workspace.
          </p>
        </div>
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="px-3 py-1 rounded-lg border border-current/20 hover:border-current/60 text-xs font-semibold cursor-pointer shrink-0"
      ...
      <section className="space-y-3" id="acc-sec-text-reading" aria-labelledby="acc-sec-text-title">
        <div className="flex items-center gap-2 text-xs font-semibold opacity-70" id="acc-sec-text-title">
          <Type className="w-4 h-4 text-[#1D9E75]" />
        ...
        <div className="space-y-2" id="acc-font-size-group">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5" id="acc-font-size-grid" role="radiogroup" aria-labelledby="acc-font-size-lbl">
            {[
              { id: 'standard', label: 'Default text size', desc: 'Baseline 14px scale' },
              { id: 'large', label: 'Large text', desc: 'Enhanced 16px scale' },
              { id: 'extra-large', label: 'Extra large text', desc: 'Maximum 18px scale' }
            ].map((opt) => {
      ...
      <section className="space-y-3 pt-4 border-t border-current/10" id="acc-sec-display" aria-labelledby="acc-sec-display-title">
        <div className="flex items-center gap-2 text-xs font-semibold opacity-70" id="acc-sec-display-title">
          <Eye className="w-4 h-4 text-[#1D9E75]" />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5" id="acc-display-grid" role="radiogroup" aria-label="Display theme mode">
          {[
            { id: 'light', label: 'Light mode', icon: Sun, desc: 'Warm off-white background' },
            { id: 'dark', label: 'Dark mode', icon: Moon, desc: 'Quiet twilight palette' },
            { id: 'high-contrast', label: 'High contrast', icon: Eye, desc: 'Black & white 21:1 contrast' },
            { id: 'low-vision', label: 'Low vision', icon: Zap, desc: 'Low-glare yellow-on-dark' }
          ].map((opt) => {
      ...
      <section className="space-y-4 pt-4 border-t border-current/10" id="acc-sec-reading-support" aria-labelledby="acc-sec-reading-title">
        <div className="flex items-center gap-2 text-xs font-semibold opacity-70" id="acc-sec-reading-title">
          <BookOpen className="w-4 h-4 text-[#1B0A3B]" />
        ...
        <div className="p-3.5 rounded-xl border border-current/20 bg-current/[0.02] flex items-center justify-between gap-4" id="acc-dyslexia-box">
          <div className="space-y-0.5" id="acc-dyslexia-txt">
            <span className="text-xs font-medium block" id="acc-dyslexia-lbl">Dyslexia-friendly font option</span>
            <span className="text-[11px] opacity-75 block" id="acc-dyslexia-desc">
              Applies Atkinson Hyperlegible character styling with enhanced character distinction to prevent letter swapping.
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="acc-reading-controls-grid">
          ...
          <div className="space-y-1.5" id="acc-letter-spacing-group">
            <label className="text-xs font-medium opacity-90 block">Increased letter spacing</label>
            <div className="flex flex-col gap-1" role="radiogroup" aria-label="Letter spacing options">
              {[
                ...
                { id: 'extra-wide', label: 'Extra wide (+0.15em)' }
              ].map((sp) => (
                <button
                  key={sp.id}
                  id={`acc-ls-${sp.id}`}
                  ...
                  className={`px-3 py-1.5 rounded-lg border text-xs text-left transition-all cursor-pointer flex justify-between items-center ${
          ...
          <div className="space-y-1.5" id="acc-line-spacing-group">
            <label className="text-xs font-medium opacity-90 block">Increased line spacing</label>
            <div className="flex flex-col gap-1" role="radiogroup" aria-label="Line height options">
              {[
                { id: 'standard', label: 'Standard height (1.625)' },
                { id: 'double', label: 'Double height (2.0)' },
                { id: 'spacious', label: 'Spacious height (2.25)' }
              ].map((lh) => (
                <button
                  key={lh.id}
                  id={`acc-lh-${lh.id}`}
          ...
          <div className="space-y-1.5" id="acc-reading-width-group">
            <label className="text-xs font-medium opacity-90 block">Adjustable reading width</label>
            <div className="flex flex-col gap-1" role="radiogroup" aria-label="Reading container width options">
              {[
                { id: 'narrow', label: 'Narrow (672px max)' },
                { id: 'standard', label: 'Standard (768px max)' },
                { id: 'wide', label: 'Wide (1024px max)' },
      ...
      <section className="space-y-3 pt-4 border-t border-current/10" id="acc-sec-interaction" aria-labelledby="acc-sec-interaction-title">
        <div className="flex items-center gap-2 text-xs font-semibold opacity-70" id="acc-sec-interaction-title">
          <Sliders className="w-4 h-4 text-[#1B0A3B]" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3" id="acc-interaction-grid">
          ...
          <div className="p-3.5 rounded-xl border border-current/20 bg-current/[0.02] flex items-center justify-between gap-3" id="acc-motion-box">
            <div className="space-y-0.5">
              ...
              <span className="text-[11px] opacity-75 block">Disables all screen sliding transitions and visual motion.</span>
            </div>
            <button
          ...
          <div className="p-3.5 rounded-xl border border-current/20 bg-current/[0.02] flex items-center justify-between gap-3" id="acc-focus-box">
            <div className="space-y-0.5">
              ...
              <span className="text-[11px] opacity-75 block">Highlights active keyboard buttons with high-contrast outlines.</span>
            </div>
            <button
              id="acc-toggle-focus"
              role="switch"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2" id="acc-extra-interaction-row">
          <div className="p-3 rounded-xl border border-current/15 flex items-center justify-between gap-2">
            <span className="text-xs font-medium flex items-center gap-2">
              {settings.soundEnabled ? <Volume2 className="w-4 h-4 text-[#1D9E75]" /> : <VolumeX className="w-4 h-4 opacity-50" />}
              Sound Feedback Cues
            </span>
            <button
              id="acc-toggle-sound"
          ...
          <div className="p-3 rounded-xl border border-current/15 flex items-center justify-between gap-2">
            <span className="text-xs font-medium">Interface Density</span>
            <div className="flex gap-1">
              {(['spacious', 'compact'] as const).map(density => (
                <button
                  ...
                  className={`px-2.5 py-1 text-xs rounded-md border font-medium cursor-pointer capitalize ${
      ...
      <section className="pt-4 border-t border-current/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4" id="acc-sec-reset">
        <div className="space-y-0.5">
          ...
          <p className="text-xs opacity-75 leading-normal max-w-md">
            Restore all font sizes, high-contrast modes, reading widths, and interaction options back to baseline values.
          </p>
        </div>
        ...
        <button
          ...
          onClick={handleReset}
          className="px-4 py-2.5 rounded-xl border border-[#912A4A]/40 text-[#912A4A] dark:text-[#D9A0B4] hover:bg-[#912A4A]/10 text-xs font-semibold flex items-center gap-2 cursor-pointer transition-colors focus-visible:ring-2 focus-visible:ring-[#912A4A]"
      ...
      {appModules.length > 0 && (
        <section className="pt-4 border-t border-current/10 space-y-2" id="acc-sec-modules">
          ...
          <p className="text-[11px] opacity-75">
            Hide unneeded modules to reduce interface visual density.
          </p>
          <div className="flex flex-wrap gap-2 pt-1" id="acc-modules-list">
            {appModules.map(moduleName => {
              const isChecked = settings.activeModules.includes(moduleName);
              return (
                <button
                  key={moduleName}
