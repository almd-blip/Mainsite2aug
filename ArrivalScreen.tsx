...
export default function ArrivalScreen({
  ...
  useEffect(() => {
    ...
    // Greeting fade-in is ~0.5s delay + 3.5s duration; hide the dot just after.
    const timer = setTimeout(() => setIndicatorVisible(false), 4300);
        ...
        <div className={`space-y-16 text-left ${showSettings ? 'lg:col-span-6' : 'lg:col-span-12'} transition-all duration-300`} id="arrival-content">
          ...
          >
            <button
              ...
              onClick={onContinue}
              className="px-8 py-4 text-base font-medium border border-current rounded-full transition-all hover:bg-current hover:text-background cursor-pointer"
            >
              {cmsText('arrival.continue', 'Continue')}
            </button>

            <button
              id="arrival-accessibility-btn"
              onClick={() => setShowSettings(!showSettings)}
              className={`px-8 py-4 text-base font-medium border rounded-full transition-all cursor-pointer flex items-center gap-2 ${
                showSettings 
                  ? 'bg-current text-background border-current font-semibold shadow-sm' 
                  : 'border-current/30 hover:border-current/60 hover:bg-current/[0.04]'
              }`}
            >
